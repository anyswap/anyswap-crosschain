...
function Balance({ balance }: { balance: any }) {
  ...
  // console.log(balance)
  return (
    <StyledBalanceText title={isBl ? balance.toExact() : balance.balance}>
      {isBl ? (balance ? balance?.toSignificant(6) : '') : balance?.balances ? balance?.balances?.toSignificant(6) : ''}
    </StyledBalanceText>
  )
}
            ...
            <FlexSC>
              ...
              {isNativeToken || isNaN(chainId) || currencyObj?.type ? (
                ...
                <>
                  ...
                  />
                  <StyledStarIcon
                    className={starTokenList?.[currencyObj.address] ? 'star' : ''}
                    onClick={event => {
              ...
              {currencyObj?.type
                ? ['swapin', 'swapout'].includes(currencyObj?.type)
                  ? ' (Bridge)'
                  : ' (Router ' + currencyObj.sortId + ')'
                : ''}
            </FlexSC>
            <Text fontSize={'10px'}>
              {currencyObj.name ? config.getBaseCoin(currencyObj.symbol, isDestChainId, 1, currencyObj.name) : ''}
            </Text>
          </Text>
        </Column>
...
export default function BridgeCurrencyList({
  ...
  const htmlNodes = useMemo(() => {
    ...
    for (const obj of itemData) {
      ...
      if (isNativeToken || obj?.address === config.getCurChainInfo(chainId)?.symbol) {
        ethNode.push(obj)
        // continue
      }
  ...
  return (
    <>
      <ListBox ref={boxRef} style={{ height: height }}>
        <LazyList records={htmlNodes} pageSize={pageSize} boxRef={boxRef} watchRef={watchRef} list={List}>
