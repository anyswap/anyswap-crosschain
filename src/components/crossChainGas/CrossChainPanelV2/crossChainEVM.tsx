...
export default function CrossChain({ bridgeKey }: { bridgeKey: any }) {
  ...
  useEffect(() => {
    if ([ChainId.XLM, ChainId.XLM_TEST].includes(selectChain) && isAddress(recipient, selectChain)) {
  ...
  useEffect(() => {
    if (selectChain === ChainId.XRP && recipient && recipient.indexOf('0x') !== 0 && destConfig?.address !== 'XRP') {
  ...
  const xrpurl = useMemo(() => {
    let url = ''
    if (selectChain === ChainId.XRP && destConfig?.address !== 'XRP') {
  ...
  function CrossChainTip() {
    if (
      ...
      if (xlmlimit === 'INIT') {
        return <ConfirmText>Loading...</ConfirmText>
      }
      return <ConfirmText>Get trust set error, the transaction may fail.Please set Trustlines.</ConfirmText>
    } else if (selectChain === ChainId.XRP && xrplimit === 'NOPASS') {
      return (
        <ConfirmText>
          Get trust set error, the transaction may fail.Please use{' '}
          <a href={xrpurl} target="__blank">
            {xrpurl}
          </a>
        </ConfirmText>
      )
      ...
      if (window?.near?.account()) {
        return (
          <ConfirmText>
            Get storage balance of receive account error, the transaction may fail.Please deposit near to the
            token&apos;s storage.
          </ConfirmText>
        )
      } else {
        return <ConfirmText>Please connect wallet or install Sender Wallet.</ConfirmText>
      }
    } else if (!solTokenAddress && [ChainId.SOL, ChainId.SOL_TEST].includes(selectChain)) {
      if (window?.solana) {
        return <ConfirmText>Please create address.</ConfirmText>
      } else {
        return <ConfirmText>Please open or install Solana wallet.</ConfirmText>
      }
    } else if (
      [ChainId.APT, ChainId.APT_TEST].includes(selectChain) &&
        ...
        return (
          <ConfirmText>
            {aptRegisterList.error
              ? aptRegisterList.error
              : 'Please register token first. Token registration is Aptos security policy, which required in your first interaction with an asset.'}
          </ConfirmText>
        )
      } else {
        return <ConfirmText>Please open or install Petra wallet.</ConfirmText>
      }
    } else {
      let otherTip: any
  ...
  function ViemConfirmBtn() {
    if (!evmAccount) {
      ...
      if (window?.near?.account()) {
        return (
          <ButtonPrimary
            disabled={!isAddress(recipient, selectChain)}
            onClick={() => {
              depositStorageNear(destConfig?.address, recipient, nearStorageBalanceBounds)
                .then(() => {
      ...
      if (window?.solana) {
        return (
          <ButtonPrimary
            disabled={!isAddress(recipient, selectChain)}
            onClick={() => {
              createAccount({ token: destConfig?.address, account: recipient, chainId: selectChain })
                .then(() => {
          ...
          <>
            ...
            {!aptRegisterList?.[destConfig?.address] && !aptRegisterList.error ? (
              <ButtonPrimary
                ...
                style={{ margin: '0 5px' }}
                onClick={() => {
                  setAptAllowance(destConfig?.address, selectChain, recipient, destConfig?.anytoken?.address)
                    .then(() => {
            ...
            {!aptRegisterList?.[destConfig?.anytoken?.address] && !aptRegisterList.error ? (
              <ButtonPrimary
                ...
                style={{ margin: '0 5px' }}
                onClick={() => {
                  setAptAllowance(destConfig?.anytoken?.address, selectChain, recipient, destConfig?.anytoken?.address)
                    .then(() => {
                      alert('Register success.')
                    })
                    .catch((error: any) => {
