...
export function useInitSelectCurrency(allTokensList: any, useChainId: any, initToken?: any, onlyUnderlying?: any) {
  ...
  return useMemo(() => {
    ...
    if (initToken) {
      ...
      config.getCurChainInfo(useChainId)?.bridgeInitToken ||
      config.getCurChainInfo(useChainId)?.crossBridgeInitToken
    ) {
...
export function useDestChainid(
  ...
  useEffect(() => {
    ...
    if (selectCurrency) {
      ...
      if (arr.length > 0) {
        if (!useChain || (useChain && !arr.includes(useChain.toString()))) {
          for (const c of arr) {
...
export function useDestCurrency(selectCurrency: any, selectDestCurrencyList: any) {
...
export function getFTMSelectPool(selectCurrency: any, chainId: any, selectChain: any, destConfig: any) {
