...
import { ListBox } from '../../pages/styled'
        ...
        getP2PInfo(recipient, selectChain, selectCurrency?.symbol, selectCurrency?.address).then((res: any) => {
          ...
          if (res?.p2pAddress) {
            ...
            if (res?.p2pAddress === localAddress && isAddress(localAddress, chainId)) {
              ...
              setLocalConfig(recipient, selectCurrency?.address, selectChain, CROSSCHAINBRIDGE, {
    ...
    if (memo) {
          ...
          <>
            <div className="item">
              ...
              <p className="value flex-bc">
                {indexStr}
                <CopyHelper toCopy={indexStr} />
              </p>
            </div>
            <div className="item">
              <p className="label">Data:</p>
              <p className="value flex-bc">
                {shortenAddress(dataStr, 8)}
                <CopyHelper toCopy={dataStr} />
              </p>
            </div>
          </>
        )
      ...
      return (
        <div className="item">
          ...
          <p className="value flex-bc">
            {shortenAddress(memo, 8)}
            <CopyHelper toCopy={memo} />
          </p>
        </div>
      )
    }
    ...
    if ([ChainId.IOTA, ChainId.IOTA_TEST].includes(chainId)) {
      return (
        <ButtonLight
          onClick={() => {
            window.open(
              `iota://wallet/swapOut/${routerToken}/?amount=${inputBridgeValue}&unit=Mi&chainId=${selectChain}&receiverAddress=${recipient}`
            )
          }}
        >
          {t('Confirm')}
      ...
      >
        <ListBox>
          {[ChainId.IOTA, ChainId.IOTA_TEST].includes(chainId) ? (
            <>
              <CrossChainTip>
                Please use IOTA wallet to transfer IOTA token to deposit address and input receive address on dest chain
                as input.
          ...
          {[ChainId.XRP].includes(chainId) ? (
            <>
              <CrossChainTip>
                Please use XRP wallet to transfer XRP token to deposit address and input receive address on dest chain
                as memo.
