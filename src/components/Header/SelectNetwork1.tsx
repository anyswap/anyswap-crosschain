...
import { TabList } from '../CurrencySelect/styleds'
...
console.info('spportChainArrspportChainArr======', spportChainArr)
        ...
        <WalletLogoBox2>
          <div className="left" onClick={() => changeNetwork(chainInfo[curChainId])}>
            ...
            <OptionCardLeft id={'chain_list_name_' + curChainId}>
              <HeaderText>
                {' '}
                {curChainId && selectChainId && curChainId.toString() === selectChainId.toString() ? (
                  <CircleWrapper>
                    <GreenCircle>
                      <div />
                    </GreenCircle>
            ...
            <OptionCardLeft1 id={'chain_list_url_' + curChainId} onClick={e => e.stopPropagation()}>
              <Input
                value={viewUrl}
                id={'chain_list_input_' + curChainId}
                onChange={(event: any) => {
          ...
          <div className="right">
            ...
            {viewLoading ? (
              ...
              <CheckSquareIcon
                id={'chain_list_tick_' + curChainId}
                onClick={e => {
                  setViewLoading(true)
                  const htmlNameNode = document.getElementById('chain_list_name_' + curChainId)
                  const htmlNameNode1 = document.getElementById('chain_list_set_' + curChainId)
            ...
            <StyledStarIcon
              className={starChainList?.[curChainId] ? 'star' : ''}
        ...
        {records?.map((item: any, index: any) => {
          if (
            ...
            return (
              <OptionCardClickable
                key={index}
                className={useChainId?.toString() === item.chainID?.toString() ? 'active' : ''}
              >
                <Option
                  curChainId={item.chainID}
                  selectChainId={useChainId}
                  changeNetwork={val => {
                    openUrl(val)
                  }}
                ></Option>
    ...
    return (
      ...
      >
        <Column style={{ width: '100%', flex: '1 1' }}>
          ...
          <Separator />
          <TabList>
            <div className={'item ' + (starTabIndex === 0 ? 'active' : '')} onClick={() => onChangeStarTab(0)}>
              My Favorites
            </div>
