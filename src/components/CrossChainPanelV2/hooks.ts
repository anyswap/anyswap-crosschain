...
export function useInitSelectCurrency(allTokensList: any, useChainId: any, initToken?: any) {
  console.log('useInitSelectCurrency', { allTokensList, useChainId, initToken })

  return useMemo(() => {
    const t = initToken
      ? [initToken]
      : [
          config.getCurChainInfo(useChainId)?.bridgeInitToken?.toLowerCase(),
          config.getCurChainInfo(useChainId)?.crossBridgeInitToken?.toLowerCase()
        ]
    ...
    console.log('initCurrency', initCurrency)
...
export function useDestChainid(selectCurrency: any, selectChain: any, useChainId: any) {
  return useMemo(() => {
    let initChainId: any = '',
      initChainList: any = []
    if (selectCurrency) {
